...
#%%
def liveHeli(ejex,horas,escala,fili,filf,markersize):
    import sys
    ...
    import numpy as np
    volcan =gdb.get_metadata_volcan('*',rep='y')
    volcan = volcan.drop_duplicates(subset='nombre', keep="first")
    for index,rowv in volcan.iterrows():
        estas = gdb.get_metadata_wws(volcan=rowv.nombre_db, est=None, estadoEst='on', estadoInst='on')
        ...
        if len(amps)>0:
            ...
            for i in range(0,len(filas)):
                ...
                if i==0:
                    titulo = 'Estación '+esta0+' - '+rowv.vol_tipocorto+' '+rowv.nombre
        ...
        plt.savefig('//172.16.40.10/sismologia/liveHeli/'+rowv.nombre_db+'.png')
        plt.close('all')
...
liveHeli(ejex, horas, escala, fili, filf, markersize)